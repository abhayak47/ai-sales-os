...
export default function DealRisks() {
    ...
    <div className="border border-red-500/30 bg-red-500/5 rounded-xl p-5 mb-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-base font-bold text-red-400">
            🚨 Deal Risk Alert — {data.total} deal{data.total > 1 ? "s" : ""} at risk
          </h2>
          <p className="text-white/40 text-xs mt-1">
            These deals need your attention today
          </p>
        </div>
        <button
          onClick={fetchRisks}
      ...
      <div className="space-y-3">
        {data.risks.map((risk, i) => (
          <div
            key={risk.lead_id}
            className="border border-white/10 bg-black rounded-xl overflow-hidden"
          >
            {/* Risk Header */}
            <div
            ...
            >
              <div className="flex items-center gap-3">
                <span className="text-sm">{risk.risk_level}</span>
                <div>
                  <div className="font-medium text-sm">{risk.lead_name}</div>
                  {risk.company && (
                    <div className="text-white/40 text-xs">{risk.company}</div>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-3">
            ...
            {expanded === i && (
              <div className="px-4 pb-4 space-y-3 border-t border-white/10 pt-3">
                <div className="text-sm text-red-400/80">{risk.risk_reason}</div>
